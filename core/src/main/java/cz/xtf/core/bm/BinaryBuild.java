...
public abstract class BinaryBuild implements ManagedBuild {
	...
	public void delete(OpenShift openShift) {
		...
		new SimpleWaiter(() -> openShift.getImageStream(is.getMetadata().getName())  == null, TimeUnit.MILLISECONDS, WaitingConfig.timeout(), "Waiting for old imageStreams deletion before creating new one").waitFor();
		new SimpleWaiter(() -> openShift.getBuildConfig(bc.getMetadata().getName())  == null, TimeUnit.MILLISECONDS, WaitingConfig.timeout(), "Waiting for old buildConfigs deletion before creating new one").waitFor();
		new SimpleWaiter(() -> openShift.getPods().stream().noneMatch(p -> (podName).equals(p.getMetadata().getName())), TimeUnit.MILLISECONDS, WaitingConfig.timeout(), "Waiting for old pods deletion before creating new one").waitFor();
	}
